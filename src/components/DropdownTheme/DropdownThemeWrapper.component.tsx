import React, { FocusEventHandler, ReactElement, ReactNode, useState } from 'react';
...
import DropdownTheme, { DropdownThemeDefaultProps } from './DropdownTheme.component';
import { StyledDropdownSelector, StyledDropdownThemeContainer, StyledMenuContainer } from './DropdownTheme.styled';
...
const DropDownContainer = ({ target, isOpen, children, onClose }: DropDownContainerProps) => {
    return (
...
const DropdownThemeWrapper = ({
    ...
    const removeItem = (item: any, event: any) => {
        ...
        if (Array.isArray(value) && value.length > 0 && item && item.value && item.label) {
            const tempValue: any = value.filter((row: any) => row.value !== item.value && row.label !== item.label);
            onChange(tempValue);
        } else {
            onChange(null);
        }
    ...
    const renderSelectorIcons = () => {
        return (
            <div className="iconContainer">
                {renderClearableIcon()}
                <div className={`${isOpen ? 'downIconContainerTransition' : ''} downIconContainer`}>
            ...
            target={
                ...
                >
                    <div className="selectContainer">
                        <div className={`labelContainer ${isLabelVisible ? 'labelTransition' : ''}`}>
                            {renderSelectedValue()}
                        </div>
