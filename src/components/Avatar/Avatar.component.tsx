...
function AvatarComponent({
    ...
    if (isSquare) {
        return (
            ...
            >
                {shouldShowTwoInitials ? (title ? getTwoNameInitials(title) : icon) : getNameInitials(title)}
            </StyledAvatar>
        );
    }
    return (
        <>
            {isTooltip ? (
                <Tooltip title={title}>
                    {url && url.length > 0 ? (
                        <StyledAvatarContainer width={width} height={height} src={url} alt=""></StyledAvatarContainer>
                    ) : (
