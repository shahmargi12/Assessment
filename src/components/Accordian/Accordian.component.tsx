...
function AccordionComponent({
  ...
  return (
    <StyledAccordionContainer>
      ...
      >
        <AccordionSummary
          expandIcon={<Icons.DownArrowIcon height={8} width={14} />}
        >
          <StyledAccordionTypography className="email-address-cell">
            <div className="userNameChar">
              <Avatar
                url={
                  dataRow.userAvatar ? dataRow.userAvatar : profilePlaceholder
                }
                size="medium"
                isAvatarColor
              />
          ...
          <StyledAccordionTypography className="role-cell">
            {dataRow?.roleName}
          </StyledAccordionTypography>
          <StyledAccordionTypography
            className={
              dataRow?.statusLabel === "Inactive" ? "disabled" : "status-cell"
            }
            onClick={(event) => event.stopPropagation()}
            onFocus={(event) => event.stopPropagation()}
          >
            {dataRow?.statusLabel === "Pending" ||
            dataRow?.statusLabel === "invited-active" ? (
              <Button
                variant="text"
                onClick={() => onResendInvitation(dataRow)}
              >
                {capitalize("Resend Invite")}
              </Button>
            ) : (
              capitalize(
                dataRow?.statusLabel === "invited-inactive"
                  ? "inactive"
                  : dataRow.statusLabel
              )
            )}
