...
import { StyledChip, StyledInputContainer, StyledErrorContainer, StyleMinHeight } from './InputChip.styled';
...
const InputChip = ({
    ...
    const handleCurrentValue = () => {
        ...
        } else if (!currentValue && value && value.length > 0 && value.length <= maxLimit) {
    ...
    const removeItem = (item: string, index: number) => {
        const tempValue: string[] = value.filter((row: string, i) => row !== item || index !== i);
        onChange(tempValue);
    };
        ...
        <>
            ...
            >
                <StyleMinHeight>
                    ...
                    {isError && errorMessage && <StyledErrorContainer>{errorMessage}</StyledErrorContainer>}
                </StyleMinHeight>
            </ClickAwayListener>
        </>
    );
